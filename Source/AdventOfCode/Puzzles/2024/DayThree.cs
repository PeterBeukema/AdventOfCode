...
public class DayThree : DailyPuzzle
{
    ...
    {
        var multiplications = GetMultiplications();

        SolvePartOne(multiplications);
        SolvePartTwo(multiplications);
    }

    private void SolvePartOne(List<(int index, int first, int second)> multiplications)
    {
        var result = 0;

        foreach (var multiplication in multiplications)
        {
            var sum = multiplication.first * multiplication.second;

            result += sum;
        }

        Console.WriteLine("Day 3, Part One: " + result);
    }

    private void SolvePartTwo(List<(int index, int first, int second)> multiplications)
    {
        var disabledRanges = GetDisabledRanges();

        // Remove all entries in 'multiplications' that are between 'dontIndexes' and 'doIndexes'
        // Then solve the same way as in SolvePartOne

        foreach (var disabledRange in disabledRanges)
        {
            multiplications.RemoveAll(multiplication => multiplication.index >= disabledRange.start && multiplication.index < disabledRange.end);
        }
        ...
        Console.WriteLine("Day 3, Part Two: " + result);

    }

    private static List<(int index, int first, int second)> GetMultiplications()
    {
        var matches = Regex.Matches(Data, @"mul\((\d{1,3}),(\d{1,3})\)");
        ...
        foreach (Match match in matches)
        {
            ...
            result.Add((index, first, second));
        }

        return result;
    }

    private static List<(int start, int end)> GetDisabledRanges()
    {
        var doIndexes = GetDoIndexes();
        var dontIndexes = GetDontIndexes();

        var result = new List<(int, int)>();

        foreach(var dontIndex in dontIndexes)
        {
            var nextDoIndex = doIndexes.FirstOrDefault(index => index > dontIndex);

            if (nextDoIndex == 0)
            {
                nextDoIndex = Data.Length;
            }

            result.Add((dontIndex, nextDoIndex));
        }

        return result;
    }

    private static List<int> GetDoIndexes()
    {
        var matches = Regex.Matches(Data, @"do\(\)");

        return matches.Select(match => match.Index).Order().ToList();
    }

    private static List<int> GetDontIndexes()
    {
        var matches = Regex.Matches(Data, @"don't\(\)");
